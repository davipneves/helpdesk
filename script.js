...
function renderizarTela() { 
    ...
    switch (telaAtual) {
        ...
        case 'dashboard':
            ...
            break
        case 'novo-ticket': setupNovoTicket(); break
        case 'detalhe-ticket': setupDetalheTicket(); break
         ...
         tickets.forEach(function(ticket) {
            ...
            tr.innerHTML = '<td class="px-6 py-4 whitespace-nowrap"><div class="text-sm font-medium text-gray-900">' + ticket.assunto + '</div><div class="text-sm text-gray-500">#' + ticket.id + '</div></td>' +
             ...
             tr.addEventListener('click', function() { 
                //console.log('foi no dashboard:', ticket.id) //teste 
...
function setupDetalheTicket() {
    //console.log('tela carregada:', ticketSelecionadoId) //teste
    ...
    var selectUrgencia = document.getElementById('detalhe-urgencia')
    var selectImpacto = document.getElementById('detalhe-impacto')
    ...
    preencherSelect(selectUrgencia, OPCOES_URGENCIA)
    preencherSelect(selectImpacto, OPCOES_IMPACTO)
    ...
    Promise.all([promessaTicket, promessaTecnicos])
        .then(function(resultados) {
            ...
            var elDepartamento = document.getElementById('detalhe-departamento')
            ...
            if (elDepartamento) elDepartamento.textContent = ticket.nomeDepartamento || 'N/A'
            ...
            if(selectUrgencia) selectUrgencia.value = ticket.urgencia
            if(selectImpacto) selectImpacto.value = ticket.impacto
            ...
            var camposEditaveis = [selectEstado, selectUrgencia, selectImpacto, selectPrioridade, selectTecnico]
            ...
            if (tipoUsuario === 'tecnico' && btnSalvar) {
                ...
                btnSalvar.addEventListener('click', function() {
                    ...
                    var dadosAtualizados = {
                        id: ticketSelecionadoId, 
                        estado: selectEstado.value, 
                        urgencia: parseInt(selectUrgencia.value),
                        impacto: parseInt(selectImpacto.value),
                        prioridade: parseInt(selectPrioridade.value), 
...
function setupHistorico() {
    ...
    function renderizarTabelaHistorico(tickets) {
        ...
        if (!tickets || tickets.length === 0) {
            var colunas = (tipoUsuario === 'tecnico') ? 7 : 6
        ...
        tickets.forEach(function(ticket) {
            ...
            tr.innerHTML =
                ...
                '<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600"' + tooltipExclusao + '>' + (ticket.nomeDepartamento || 'N/A') + '</td>' + 
