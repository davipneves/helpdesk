...
var PRIORIDADES_TICKET = [{ id: 'baixa', nome: 'Baixa' }, { id: 'media', nome: 'Média' }, { id: 'alta', nome: 'Alta' }, { id: 'urgente', nome: 'Urgente' }]
var IMPACTO_OPCOES = [{ id: 'baixo', nome: 'Baixo' }, { id: 'medio', nome: 'Médio' }, { id: 'alto', nome: 'Alto' }]
var URGENCIA_OPCOES = [{ id: 'baixa', nome: 'Baixa' }, { id: 'media', nome: 'Média' }, { id: 'alta', nome: 'Alta' }]
...
function apiFetch(action, body) {
    ...
    return fetch('api.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
        .then(function (response) { if (!response.ok) { throw new Error('Erro de rede ou servidor: ' + response.statusText); } return response.json(); })
        .then(function (data) { if (data.success === false) { throw new Error(data.message || 'Erro desconhecido da API'); } return data; })
}
function formatarData(stringData) { if (!stringData) return 'N/A'; var data = new Date(stringData); return data.toLocaleString('pt-BR'); }
function mostrarErro(mensagem) { var template = document.getElementById('template-modal-erro'); var clone = template.content.cloneNode(true); clone.getElementById('modal-erro-mensagem').textContent = mensagem; var fecharBtn = clone.getElementById('modal-erro-fechar'); fecharBtn.addEventListener('click', fecharErro); modalContainer.innerHTML = ''; modalContainer.appendChild(clone); }
function fecharErro() { modalContainer.innerHTML = ''; }
function preencherSelect(selectElement, options, valorPadrao, chaveId, chaveNome) { if (!selectElement) return; chaveId = chaveId || 'id'; chaveNome = chaveNome || 'nome'; selectElement.innerHTML = ''; options.forEach(function (op) { var option = document.createElement('option'); option.value = op[chaveId]; option.textContent = op[chaveNome]; if (op[chaveId] === valorPadrao) { option.selected = true; } selectElement.appendChild(option); }) }
function renderizarHeader() { var template = document.getElementById('template-header'); var clone = template.content.cloneNode(true); clone.getElementById('header-nome-usuario').textContent = usuarioAtual.nome.split(' ')[0]; clone.getElementById('header-titulo-portal').textContent = '| ' + (tipoUsuario === 'funcionario' ? 'Portal do Funcionário' : 'Portal do Técnico'); clone.getElementById('btn-logout').addEventListener('click', fazerLogout); return clone; }
    ...
    switch (telaAtual) {
        ...
        case 'dashboard':
            if (tipoUsuario === 'funcionario'){
                setupDashboardFuncionario()
            } 
            else {
                setupDashboardTecnico()
            }   
            break
        case 'novo-ticket': 
            setupNovoTicket() 
...
function setupTelaLogin() {
    ...
    form.addEventListener('submit', function (e) {
        e.preventDefault(); erroLogin.textContent = ''; btnSubmit.disabled = true; btnSubmit.innerHTML = '<div class="spinner !w-6 !h-6 !border-2"></div>'
...
function setupDashboardFuncionario() {
    ...
    apiFetch('getMeusTickets', { codFuncionario: usuarioAtual.codFuncionario }) 
        .then(function (data) {
            tbody.innerHTML = ''
            ...
            data.tickets.forEach(function (ticket) {
                ...
                tr.innerHTML = '<td class="px-6 py-4 whitespace-nowrap"><div class="text-sm font-medium text-gray-900">' + ticket.assunto + 
                    '</div><div class="text-sm text-gray-500">#' + ticket.id + '</div></td>' +
                    '<td class="px-6 py-4 whitespace-nowrap"><span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ' + corEstado + '">' + estado.nome + 
                    '</span></td>' + '<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">' + formatarData(ticket.dataAbertura) + '</td>' +
...
function setupDashboardTecnico() {
    ...
    function renderizarTabela(tbody, tickets) {
        ...
        tickets.forEach(function (ticket) {
            ...
            var prioridade = PRIORIDADES_TICKET.find(function (p) { return p.id === ticket.prioridade; }) || { id: ticket.prioridade, nome: ticket.prioridade };
            var corPrioridade = (prioridade.id === 'alta' || prioridade.id === 'urgente') ? 'text-red-600' : prioridade.id === 'media' ? 'text-yellow-600' : 'text-green-600';
            tr.innerHTML = '<td class="px-6 py-4 whitespace-nowrap"><div class="text-sm font-medium text-gray-900">' + ticket.assunto + '</div><div class="text-sm text-gray-500">#' + ticket.id + '</div></td>' +
                ...
                '<td class="px-6 py-4 whitespace-nowrap text-sm font-medium ' + corPrioridade + '">' + prioridade.nome + '</td>'
                tr.addEventListener('click', function () { navegarPara('detalhe-ticket', ticket.id); 
            })
            tbody.appendChild(tr)
        })
    ...
    apiFetch('getFilaTecnico', { codTecnico: usuarioAtual.codTecnico }) 
...
function setupNovoTicket() {
    ...
    preencherSelect(selectUrgencia, URGENCIA_OPCOES, '1 - baixa'); preencherSelect(selectImpacto, IMPACTO_OPCOES, '1 - baixo');
    apiFetch('getDepartamentos')
        .then(function (data) { preencherSelect(selectDep, data.departamentos, usuarioAtual.codDepartamento, 'codigo', 'nome'); })
        .catch(function (err) { console.error("Erro ao buscar departamentos:", err); mostrarErro("Não foi possível carregar os departamentos. Tente novamente."); });
    form.addEventListener('submit', function (e) {
        e.preventDefault(); btnSubmit.disabled = true; btnSubmit.innerHTML = '<div class="spinner !w-6 !h-6 !border-2"></div>';
        var novoTicket = {
            assunto: document.getElementById('assunto').value, descricao: document.getElementById('descricao').value, palavras_chave: document.getElementById('palavras_chave').value,
            codSolicitante: usuarioAtual.codFuncionario, nomeSolicitante: usuarioAtual.nome, codDepartamentoOrigem: selectDep.value,
            urgencia: selectUrgencia.value, impacto: selectImpacto.value, prioridade: 'media',
        };
        apiFetch('criarTicket', novoTicket)
            .then(function (data) { navegarPara('dashboard'); })
            .catch(function (err) { 
                console.error("Erro ao criar ticket:", err); mostrarErro("Falha ao criar o ticket. Tente novamente.")
                btnSubmit.disabled = false; btnSubmit.innerHTML = 'Enviar Chamado'
            })
    })
}
...
function setupDetalheTicket() {
    ...
    preencherSelect(selectPrioridade, PRIORIDADES_TICKET)
    ...
    if (tipoUsuario === 'tecnico') {
        promessaTecnicos = apiFetch('getTecnicos')
            .then(function (data) {
                ...
                selectTecnico.innerHTML = '<option value="">Não atribuído</option>'
                preencherSelect(selectTecnico, listaTecnicosCache, null, 'codTecnico', 'nome')
    ...
    Promise.all([promessaTicket, promessaTecnicos])
        .then(function (resultados) {
            ...
            document.getElementById('detalhe-id').textContent = 'Ticket #' + ticket.id;
            document.getElementById('detalhe-assunto').textContent = ticket.assunto;
            document.getElementById('detalhe-descricao').textContent = ticket.descricao;
            document.getElementById('detalhe-solicitante').textContent = ticket.nomeSolicitante;
            document.getElementById('detalhe-data').textContent = formatarData(ticket.dataAbertura);
            document.getElementById('detalhe-urgencia').textContent = URGENCIA_OPCOES.find(function (o) { return o.id === ticket.urgencia; })?.nome || ticket.urgencia;
            document.getElementById('detalhe-impacto').textContent = IMPACTO_OPCOES.find(function (o) { return o.id === ticket.impacto; })?.nome || ticket.impacto;
            if (ticket.palavras_chave) { document.getElementById('detalhe-palavras-chave').textContent = ticket.palavras_chave; document.getElementById('container-palavras-chave').classList.remove('hidden'); }

            selectEstado.value = ticket.estado
            selectPrioridade.value = ticket.prioridade
            selectTecnico.value = ticket.codTecnico || "";

            var btnSalvar = document.getElementById('btn-salvar-ticket');


            if (ticket.ativo == 0 || ticket.ativo == false) {
                alertaInativo.classList.remove('hidden')
                [selectEstado, selectPrioridade, selectTecnico].forEach(function (campo) { campo.disabled = true; })
                return
            }
            ...
            if (tipoUsuario === 'tecnico') {
                ...
                btnSalvar.addEventListener('click', function () {
                    ...
                    var tecnicoSelecionado = listaTecnicosCache.find(function (t) { return t.codTecnico === selectTecnico.value; }) || null;
                    var dadosAtualizados = {
                        id: ticketSelecionadoId, estado: selectEstado.value, prioridade: selectPrioridade.value,
                        codTecnico: tecnicoSelecionado ? tecnicoSelecionado.codTecnico : null,
                        nomeTecnico: tecnicoSelecionado ? tecnicoSelecionado.nome : null,
                        codEquipe: tecnicoSelecionado ? tecnicoSelecionado.codEquipe : null,
                    };
        ...
        .catch(function (err) {
            ...
            mostrarErro("Não foi possível carregar os detalhes do ticket.");
        });
};
            ...
            btn.addEventListener('click', function (e) {
                ...
                if (confirm('Tem certeza que deseja "excluir" (desativar) o ticket #' + idParaDesativar + '?')) {
                    ...
                    apiFetch('desativarTicket', {
                        ...
                        .catch(function (err) {
                            mostrarErro(err.message || "Falha ao desativar ticket.");
                        });
                }
            });
        });
    ...
    function buscarHistorico(filtro, mostrarInativos) {
        ...
        containerFiltros.querySelectorAll('.filtro-btn').forEach(function (btn) {
            if (btn.getAttribute('data-filtro') === filtroAtivo) {
                btn.classList.add('bg-blue-600', 'text-white'); btn.classList.remove('bg-gray-200', 'text-gray-700');
            } else {
                btn.classList.remove('bg-blue-600', 'text-white'); btn.classList.add('bg-gray-200', 'text-gray-700');
            }
        });

        apiFetch('getHistoricoTickets', {
            filtro: filtroAtivo,
            mostrarTodos: mostrarInativos 
        })
...
function setupRelatorio() {
    ...
    document.getElementById('btn-voltar-dashboard-relatorio').addEventListener('click', function () {
        navegarPara('dashboard');
    });
    ...
    apiFetch('getRelatorioComJoin')
        .then(function (data) {
            tbody.innerHTML = '';
            if (!data.relatorio || data.relatorio.length === 0) { tbody.innerHTML = '<tr><td colspan="5" class="p-8 text-center text-gray-500">Nenhum ticket ativo encontrado.</td></tr>'; return; }
    ...
    return apiFetch('login', { email: email.toLowerCase().trim(), tipo: tipo })
        .then(function (data) {
            if (data.success && data.usuario) {
                usuarioAtual = data.usuario; tipoUsuario = tipo;
...
function fazerLogout() {
    usuarioAtual = null; tipoUsuario = null; ticketSelecionadoId = null;
...
window.onload = function () {
    var usuarioSalvo = null; var tipoSalvo = null;
    try { usuarioSalvo = localStorage.getItem('helpdesk_usuario'); tipoSalvo = localStorage.getItem('helpdesk_tipoUsuario'); } catch (e) { console.error("Erro ao ler localStorage:", e); navegarPara('login'); return; }
    if (usuarioSalvo && tipoSalvo) {
        try { usuarioAtual = JSON.parse(usuarioSalvo); tipoUsuario = tipoSalvo; navegarPara('dashboard'); } catch (e) { console.error('Erro ao carregar sessão salva (JSON inválido):', e); localStorage.removeItem('helpdesk_usuario'); localStorage.removeItem('helpdesk_tipoUsuario'); navegarPara('login'); }
